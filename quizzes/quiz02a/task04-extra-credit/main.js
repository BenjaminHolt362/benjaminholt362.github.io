...
const findMatch = (song) =>
{
    let match = false;
    if (song.name.toLowerCase().includes(searchTerm.toLowerCase()))
    {
        match = true;
        //console.log("Song name " + song.name + " includes query " + searchTerm);
    }
    if (song.album.name.toLowerCase().includes(searchTerm.toLowerCase()))
    {
        match = true;
        //console.log("Song album " + song.album.name + " includes query " + searchTerm);
    }
    if (song.artist.name.toLowerCase().includes(searchTerm.toLowerCase()))
    {
        match = true;
        //console.log("Song artist " + song.artist.name + " includes query " + searchTerm);
    }
    return match;
}
...
function showMatchingTracks()
{
    ...
    let matchingTracks = tracks.filter(findMatch);
    container.innerHTML = "";

    for(let song of matchingTracks)
    {
       let snippet = `<section class="track">
